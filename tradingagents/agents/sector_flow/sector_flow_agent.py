...
SECTOR_FLOW_SUMMARY_ITEM_SPECS = (
    ("theme_strength", "板块强弱"),
    ("theme_heat", "热度"),
    ("crowding", "拥挤"),
    ("stock_role_in_theme", "个股位置"),
)
...
def _normalize_module_summary_items(value: Any) -> list[dict[str, str]]:
    ...
    return normalized[: len(SECTOR_FLOW_SUMMARY_ITEM_SPECS)]


def _is_valid_module_summary_items(value: Any) -> bool:
    if not isinstance(value, list) or len(value) != len(SECTOR_FLOW_SUMMARY_ITEM_SPECS):
        return False
    for item, (expected_key, expected_label) in zip(value, SECTOR_FLOW_SUMMARY_ITEM_SPECS, strict=True):
        if not isinstance(item, dict):
            return False
...
class SectorFlowAgent:
                    ...
                    messages.append(
                        HumanMessage(
                            content=(
                                ...
                                "6. module_brief 必须包含 conclusion_zh(2-8字) 与 rationale_zh(12-24字)；\n"
                                "7. module_summary_items 必须按固定顺序返回4条：theme_strength/板块强弱、theme_heat/热度、crowding/拥挤、stock_role_in_theme/个股位置；\n"
    ...
    def _parse_json_response(self, content: str, bundle: dict[str, Any]) -> dict[str, Any]:
        parsed = parse_json_object(content, source="SectorFlowAgent output")
